...
import { FeaturedArticle } from "../blog/featured-article";
  ...
  return (
    <div className="flex flex-col gap-6 my-section">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <FeaturedArticle article={BLOG_ARTICLES[0]} />

        <div className="flex flex-col gap-6 h-full justify-between">
          {BLOG_ARTICLES.map((article, idx) => idx !== 0 ? <Article key={idx} article={article} /> : null)}
        </div>
      </div>
    </div>
  );
